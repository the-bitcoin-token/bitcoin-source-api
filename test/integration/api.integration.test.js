...
import { BchInsightApi, BCH_BLOCKDOZER_TESTNET_URL } from '../../src'
...
TEST_APIS.forEach(({ name, api, testAddress }) => {
  describe(name, () => {
    describe('getAddress', () => {
    ...
    describe('getRawBlock', () => {
      it('Should retrieve the raw block for a given block hash', async () => {
        const res = await api.getRawBlock(
      ...
      it('Should parse a block to JSON', async () => {
        const blockHash =
          '000000000933ea01ad0ee984209779baaec3ced90fa3f408719526f8d77f4943'
        const res = await api.getRawBlock(blockHash)
